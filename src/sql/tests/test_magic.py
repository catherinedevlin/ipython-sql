...
def _setup():
    _setup_magic()
    ip.run_line_magic('sql', '-c sqlite:// CREATE TABLE test (n INT, name TEXT)')
    ip.run_line_magic('sql', "-c sqlite:// INSERT INTO test VALUES (1, 'foo');")
    ip.run_line_magic('sql', "-c sqlite:// INSERT INTO test VALUES (2, 'bar');")


def _teardown():
    ip.run_line_magic('sql', '-c sqlite:// DROP TABLE test')


@with_setup(_setup, _teardown)
def test_memory_db():
    assert ip.run_line_magic('sql', "-c sqlite:// SELECT * FROM test;")[0][0] == 1
    assert ip.run_line_magic('sql', "-c sqlite:// SELECT * FROM test;")[1]['name'] == 'bar'


@with_setup(_setup, _teardown)
def test_html():
    result = ip.run_line_magic('sql', "-c sqlite:// SELECT * FROM test;")
...
def _setup_writer():
    _setup_magic()
    ip.run_line_magic('sql', '-c sqlite:// CREATE TABLE writer (first_name, last_name, year_of_death)')
    ip.run_line_magic('sql', "-c sqlite:// INSERT INTO writer VALUES ('William', 'Shakespeare', 1616)")
    ip.run_line_magic('sql', "-c sqlite:// INSERT INTO writer VALUES ('Bertold', 'Brecht', 1956)")


def _teardown_writer():
    ip.run_line_magic('sql', "-c sqlite:// DROP TABLE writer")


@with_setup(_setup_writer, _teardown_writer)
def test_multi_sql():
    result = ip.run_cell_magic('sql', '-c sqlite://', "SELECT last_name\n"
                                                      "FROM writer;")
...
@with_setup(_setup_writer, _teardown_writer)
def test_result_var():
    ip.run_cell_magic('sql', "-c sqlite:// -s x", "SELECT last_name FROM writer;")
...
def test_access_results_by_keys():
    assert ip.run_line_magic('sql', "-c sqlite:// SELECT * FROM writer;")['William'] == (
        u'William', u'Shakespeare', 1616)


@with_setup(_setup_writer, _teardown_writer)
def test_duplicate_column_names_accepted():
    result = ip.run_line_magic('sql', "-c sqlite:// SELECT last_name, last_name FROM writer;")
...
def test_persist():
    ...
    ip.runcode("results_dframe = results.data_frame()")
...
def test_unnamed_persist():
    ...
    ip.run_line_magic('sql', '-persist results.data_frame()')
...
def test_displaylimit():
    ...
    result = ip.run_line_magic('sql', "-c sqlite:// SELECT * FROM writer;")
...
def test_bind_vars():
    ...
    result = ip.run_line_magic('sql', "-c sqlite:// SELECT :x")
